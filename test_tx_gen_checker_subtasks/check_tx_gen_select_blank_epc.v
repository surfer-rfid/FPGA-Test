...
//                                                                                     //
// Filename: check_tx_gen_select_blank_epc.v                                           //
...
task    check_tx_gen_select_blank_epc;
    ...
    begin
        ...
        $display("Computed CRC of %b at time %t",~select_vector_crc16,$realtime);
            ...
            case(state_select)
                STATE_HI_BEGIN: begin
                    if(out_i_baseband_4p5 == 1'b0)    begin
                        $display("Improper bit out during select blank epc packet at time %t",$realtime);
                        $stop;
                    end
                ...
                default: begin
                    $display("Got into a bad state in check_tx_gen_select_blank_epc at time %t",$realtime);
                    $stop;
                end
            endcase
        ...
        state_counter        =    0;
        $display("Successfully received a SELECT BLANK EPC packet at time %t",$realtime);
    end
endtask