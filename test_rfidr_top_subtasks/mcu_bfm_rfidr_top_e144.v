...
//                                                                                     //
// Filename: mcu_bfm_rfidr_top_e144.v                                                  //
...
localparam    rn16_i_mag_i_ideal    =    32'sd330000;
localparam    rn16_i_mag_q_ideal    =    -32'sd15000;
localparam    pcepc_mag_i_ideal     =    32'sd2660000;
localparam    pcepc_mag_q_ideal     =    32'sd30000;
...
begin
    ...
    wr_3byte_transaction_from_mcu_spi_cntrlr(1'b0,{4'b0001,7'b0,3'b000},8'b0000_0000,8'b0000_0011);    //We want to see the exit code as "OK" and CLK_36 as valid but not running
    $display("Checked that CLK 36 is valid at time %t",$realtime);
    
    //Make clk 36 run
    
    wr_3byte_transaction_from_mcu_spi_cntrlr(1'b1,{4'b0001,7'b0,3'b001},8'b1100_0000,8'b0000_0000);
    $display("Set CLK 36 to run at time and Use Sel %t",$realtime);
    ...
    wr_3byte_transaction_from_mcu_spi_cntrlr(1'b1,{4'b0001,7'b0,3'b000},8'b0001_0000,8'b0000_0000);
    ...
    wr_3byte_transaction_from_mcu_spi_cntrlr(1'b1,{4'b0001,7'b0,3'b000},8'b0001_0001,8'b0000_0000);
    ...
    wr_3byte_transaction_from_mcu_spi_cntrlr(1'b0,{4'b0001,7'b0,3'b001},8'b0000_0000,8'b0100_1010);
    ...
    wr_3byte_transaction_from_mcu_spi_cntrlr(1'b0,{4'b0001,7'b0,3'b000},8'b0000_0000,8'b0001_0011);
    ...
    if(rn16_i_mag_i > rn16_i_mag_i_ideal + 32'sd30000 || rn16_i_mag_i < rn16_i_mag_i_ideal - 32'sd30000)    begin
        $display("Error: RN16_I_MAG_I is off. Actual: %d Ideal %d at time %t", rn16_i_mag_i, rn16_i_mag_i_ideal, $realtime);
        $stop;
    end
    
    if(rn16_i_mag_q > rn16_i_mag_q_ideal + 32'sd6000 || rn16_i_mag_q < rn16_i_mag_q_ideal - 32'sd6000)    begin
    ...
    if(pcepc_mag_i > pcepc_mag_i_ideal + 32'sd80000 || pcepc_mag_i < pcepc_mag_i_ideal - 32'sd80000)    begin
        $display("Error: PCEPC_MAG_I is off. Actual: %d Ideal %d at time %t", pcepc_mag_i, pcepc_mag_i_ideal, $realtime);
        $stop;
    end
    
    if(pcepc_mag_q > pcepc_mag_q_ideal + 32'sd16000 || pcepc_mag_q < pcepc_mag_q_ideal - 32'sd16000)    begin
    ...
    wait(rfidr_top_e144_0.rfidr_fsm0.state==2'b00);
