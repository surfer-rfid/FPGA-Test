/////////////////////////////////////////////////////////////////////////////////////////
//                                                                                     //
// Subtask : Load Full Radio SRAM                                                      //
//                                                                                     //
// Filename: load_rfidr_top_sram_top.v                                                 //
...
task    load_rfidr_top_sram_top;
    ...
    begin
        ...
        $display("Start loading radio RAM at time %t",$realtime);
        load_rfidr_top_sram_rx;
        $display("Loaded RX RAM at time %t",$realtime);
        load_rfidr_top_sram_txcw0;
        $display("Loaded TXCW0 RAM at time %t",$realtime);
        load_rfidr_top_sram_select(select_bits,seed_epc_select_int);
        $display("Loaded select RAM at time %t",$realtime);
        load_rfidr_top_sram_select_blank_epc(select_blank_bits);
        $display("Loaded select blank RAM at time %t",$realtime);
        load_rfidr_top_sram_query(query_bits);
        $display("Loaded query RAM at time %t",$realtime);
        load_rfidr_top_sram_query_rep;
        $display("Loaded query rep RAM at time %t",$realtime);
        load_rfidr_top_sram_ack_rn16;
        $display("Loaded ack rn16 RAM at time %t",$realtime);
        load_rfidr_top_sram_ack_handle;
        $display("Loaded ack handle RAM at time %t",$realtime);
        load_rfidr_top_sram_nak;
        $display("Loaded nak RAM at time %t",$realtime);
        load_rfidr_top_sram_reqhdl;
        $display("Loaded req handle RAM at time %t",$realtime);
        load_rfidr_top_sram_reqrn16;
        $display("Loaded req rn16 RAM at time %t",$realtime);
        load_rfidr_top_sram_write(write_bits,seed_epc_write_int);
        $display("Loaded write RAM at time %t",$realtime);
        load_rfidr_top_sram_read(read_bits);
        $display("Loaded read RAM at time %t",$realtime);
        load_rfidr_top_sram_lock(lock_bits);
        $display("Loaded lock RAM at time %t",$realtime);
        $display("Finished loading radio RAM at time %t",$realtime);
    end
endtask